...
- `npm run build` 통과
- `npm run lint` 확인
- `src/lib/feature-flags.ts`의 `RELEASED_FEATURES`에 배포할 기능만 포함했는지 확인
- 로그인/회원가입/로그아웃 확인
- 보호 라우트 redirect 확인
...
- `NEXT_PUBLIC_APP_ENV=local`일 때만 모든 메뉴와 페이지가 열립니다.
- `development`, `staging`, `production`에서는 `RELEASED_FEATURES`에 등록되지 않은 기능이 사이드바에서 숨겨지고, 직접 URL 접근은 404로 차단됩니다.
- 개발 서버 빌드는 `npm run build:dev`, 운영 서버 빌드는 `npm run build`를 사용합니다.
